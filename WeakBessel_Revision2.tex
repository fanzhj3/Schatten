...
$$ R_{\lambda,k} :=\frac{\partial}{\partial x_k} \Delta_\lambda^{-\frac12},\quad k=1,\ldots,n+1. $$


As a preliminary step, we will first show that for the critical index $p=n+1$ (see \cite[Remark 1.6]{FLLXarxiv} for explanation about critical index),  the  endpoint weak Schatten class $\mathcal{L}_{n+1,\infty}(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))$ property of $[R_{\lambda,k},M_f]$  can be characterized in terms the homogeneous Sobolev space norm of $f$, i.e., $\|\nabla f\|_{L_p(\mathbb{R}_+^{n+1})}$.
...
We note that in \cite{HRpreprint}, several equivalent endpoint $\mathcal{L}_{d,\infty}$ characterizations for commutators were established recently via Haj\l asz type Sobolev spaces on spaces of homogeneous type with lower dimension $d$.
These characterizations are given in terms of the oscillation norm ${\rm OSC}_{d,\infty}$ of the symbols, or the weighted $L_{d,\infty}$ norm of the mean oscillation of the symbols, or the minimum weighted $L_d$ norm of the Haj\l asz upper gradient of the symbols. However, those
characterizations in \cite{HRpreprint} do not imply our results (Theorems \ref{main theorem} and \ref{mtc}).
\end{remark}}
...
{\color{blue} The following lemma is folklore. We provide a proof for convenience of the reader.

\begin{lemma}\label{distancelem}
Let $H$ be a Hilbert space and let $0<p<\infty.$ For every $T\in \mathcal{L}_{p,\infty}(H),$ we have
$${\rm dist}_{\mathcal{L}_{p,\infty}(H)}\Big(T,(\mathcal{L}_{p,\infty}(H))_0\Big)=\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T).$$
\end{lemma}
\begin{proof} Let $S\in(\mathcal{L}_{p,\infty}(H))_0.$ For every $\epsilon>0,$ we have
\begin{align*}
t^{\frac1p}\mu_{B(H)}(t,T)&\leq t^{\frac1p}\mu_{B(H)}(\frac{t}{1+\epsilon},T-S)+t^{\frac1p}\mu_{B(H)}(\frac{t\epsilon}{1+\epsilon},S)\\
&\leq(1+\epsilon)^{\frac1p}\|T-S\|_{p,\infty}+t^{\frac1p}\mu_{B(H)}(\frac{t\epsilon}{1+\epsilon},S).
\end{align*}
Thus,
\begin{align*}
\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T)&\leq (1+\epsilon)^{\frac1p}\|T-S\|_{p,\infty}+\limsup_{t\to\infty}t^{\frac1p}\mu_{B(H)}(\frac{t\epsilon}{1+\epsilon},S)\\
&=(1+\epsilon)^{\frac1p}\|T-S\|_{p,\infty}+(1+\epsilon^{-1})^{\frac1p}\limsup_{t\to\infty}t^{\frac1p}\mu_{B(H)}(t,S).
\end{align*}
Since $S\in (\mathcal{L}_{p,\infty}(H))_0,$ it follows that the limit at the right hand side is $0.$ Hence,
$$\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T)\leq(1+\epsilon)^{\frac1p}\|T-S\|_{p,\infty}.$$
Since $\epsilon>0$ and $S\in (\mathcal{L}_{p,\infty}(H))_0$ are arbitrary, it follows that
%$$\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T)\leq\|T-S\|_{p,\infty}.$$
%Since $S\in (\mathcal{L}_{p,\infty}(H))_0$ is arbitrary, it follows that
$$\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T)\leq {\rm dist}_{\mathcal{L}_{p,\infty}(H)}\Big(T,(\mathcal{L}_{p,\infty}(H))_0\Big).$$

To see the converse inequality, we may assume without loss of generality that $T\geq0$ and write
$$T=\sum_{k\geq0}\mu_{B(H)}(k,T)p_k,$$
where $\{p_k\}_{k\geq0}$ is a sequence of pairwise orthogonal rank one projections. Fix $\epsilon>0$ and choose $M\in\mathbb{N}$ such that
$$\sup_{t>M}t^{\frac1p}\mu_{B(H)}(t,T)\leq\epsilon+\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T).$$
Set
$$S=\sum_{k=0}^{M-1}\mu_{B(H)}(k,T)p_k.$$
We have
$$\mu_{B(H)}(t,T-S)=\mu_{B(H)}(t+M,T),\quad t>0.$$
Hence,
\begin{align*}
\|T-S\|_{p,\infty}=\sup_{t>0}t^{\frac1p}\mu_{B(H)}(t+M,T)&\leq \sup_{t>0}(t+M)^{\frac1p}\mu_{B(H)}(t+M,T)\\
&=\sup_{t>M}t^{\frac1p}\mu_{B(H)}(t,T)\leq\epsilon+\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T).
\end{align*}
In particular,
$${\rm dist}_{\mathcal{L}_{p,\infty}(H)}\Big(T,(\mathcal{L}_{p,\infty}(H))_0\Big)\leq \epsilon+\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T).$$
Since $\epsilon>0$ is arbitrary, it follows that
$${\rm dist}_{\mathcal{L}_{p,\infty}(H)}\Big(T,(\mathcal{L}_{p,\infty}(H))_0\Big)\leq\limsup_{t\to\infty}t^{\frac1{p}}\mu_{B(H)}(t,T).$$
This ends the proof of Lemma \ref{distancelem}.
\end{proof}
}
...
where $\kappa_\lambda:=\frac{1}{2^{2\lambda}\sqrt{\pi}}\frac{\Gamma(\lambda+\frac{1}{2})}{\Gamma(\lambda)}$. By functional calculus \cite{Yosida},
...
\begin{proposition}\label{commutator representation lemma} There exists a constant $\kappa_{n,\lambda}^{[3]}$ such that for every $f\in C^{\infty}_c(\mathbb{R}^{n+1})$ and for every $1\leq k\leq n+1,$
...
\end{align}
{\color{blue} Here, both sides should be understood as operators on $L_2(\mathbb{R}^{n+1}_+,m_{\lambda}).$ Furthermore, Schur multipliers should be understood as acting on (operators on) $L_2(\mathbb{R}^{n+1}_+,m_{\lambda})$ and the arguments of the Schur multipliers are operators on $L_2(\mathbb{R}^{n+1}_+,m_{\lambda}).$ Each of those arguments is of the shape $M_{x_{n+1}^{-\lambda}}TM_{x_{n+1}^{\lambda}},$ where $T$ is an operator on $L_2(\mathbb{R}^{n+1}_+)$ and $M_{x_{n+1}^{\lambda}}$ is understood as a unitary operator from $L_2(\mathbb{R}^{n+1}_+,m_{\lambda})$ to $L_2(\mathbb{R}^{n+1}_+).$ }
\end{proposition}
\begin{proof} Denote by $V_l$ the integral operator on $L_2(\mathbb{R}^{n+1}_+,m_{\lambda})$ with integral kernel
$$(x,y)\to (f(y)-f(x))K_l(x,y),\quad x,y\in\mathbb{R}_+^{n+1}.$$
{\color{blue} Using Lemma \ref{Bessel--Riesz kernel representation lemma}, we write
\begin{align}
[R_{\lambda,k},M_{E^{\ast}f}]&=\kappa_{n,\lambda}^{[2]}\mathfrak{S}_{F_{2,0}\circ H}(V_k)+\kappa_{n,\lambda}^{[2]}\delta_{k,n+1}\sum_{l=1}^{n+1}\Big(\mathfrak{S}_{h_l}\circ\mathfrak{S}_a\circ\mathfrak{S}_{F_{1,1}\circ H}\Big)(V_l)\nonumber\\
&\hspace{1.0cm}-\kappa_{n,\lambda}^{[2]}\delta_{k,n+1}\sum_{l=1}^{n+1}\Big(\mathfrak{S}_{h_l}\circ\mathfrak{S}_{h_{n+1}}\circ\mathfrak{S}_b\circ\mathfrak{S}_{F_{2,1}\circ H}\Big)(V_l)\nonumber.
\end{align}

Note that the integral kernel of the operator $R_l$ is
$$(x,y)\to\omega_n\frac{(x-y)_l}{|x-y|^{n+2}},\quad x,y\in\mathbb{R}^{n+1},$$
for some constant $\omega_{n}>0.$ Hence,
$$V_l=\frac1{\omega_n}M_{x_{n+1}^{-\lambda}} E^{\ast}[R_k,M_{f}]E M_{x_{n+1}^{\lambda}}.$$
Setting $\kappa_{n,\lambda}^{[3]}:=\omega_n^{-1}\kappa_{n,\lambda}^{[2]},$ we complete the proof.}
\end{proof}
...
{\color{blue}
\begin{lemma}\label{post-parcet-adashed lemma} Set
$$m_s(x,y)=|x-y|^{is},\quad x,y\in\mathbb{R}^{n+1},\quad s\in\mathbb{R}.$$
Then for every $1<p<\infty$, there is a constant $C_{n,p}>0$ such that
$$\|\mathfrak{S}_{m_s}\|_{\mathcal{L}_p(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}\leq C_{n,p}(1+|s|)^{\frac{n+1}{2}}.$$
\end{lemma}
\begin{proof} We may assume without loss of generality that $2<p<\infty.$
	
Theorem A' on p.1249 in \cite{MR4660138} asserts
$$\|\mathfrak{S}_{m_s}\|_{\mathcal{L}_p(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}\leq \frac{p^2}{p-1}C_{\sigma,n}\|m_s\|_{{\rm HMS}_{2,\sigma}},\quad \sigma>\frac{n+1}{2},$$
where the norm $\|\cdot\|_{{\rm HMS}_{2,\sigma}}$ is defined by equation (2.19) in \cite{MR4660138}. Since $m_s(x,y)=f_s(x-y),$ where $f_s(x)=|x|^{is}$ for $x\in\mathbb{R}^{n+1},$ it follows immediately from equation (2.19) in \cite{MR4660138} that
$$\|m_s\|_{{\rm HMS}_{2,\sigma}}=2\|\psi\cdot \sigma_{2^{j}}f_s\|_{W^{\sigma,2}(\mathbb{R}^{n+1})}=2\|\psi f_s\|_{W^{\sigma,2}(\mathbb{R}^{n+1})}$$
for some bump function $\psi$ supported on the annulus $\{\frac12\leq |x|\leq 2\}$, where $\sigma_{r}f(x):=f(rx)$. We have
$$\|\psi f_s\|_{W^{m,2}(\mathbb{R}^{n+1})}\leq c_{m,n}(1+|s|)^m,\quad m\in\mathbb{Z}_+.$$
By the Hadamard three lines theorem,
$$\|\psi f_s\|_{W^{\sigma,2}(\mathbb{R}^{n+1})}\leq \|\psi f_s\|_{W^{\lfloor\sigma\rfloor,2}(\mathbb{R}^{n+1})}^{1+\lfloor\sigma\rfloor-\sigma}\|\psi f_s\|_{W^{1+\lfloor\sigma\rfloor,2}(\mathbb{R}^{n+1})}^{\sigma-\lfloor\sigma\rfloor}\leq c_{\lfloor\sigma\rfloor,n}^{1+\lfloor\sigma\rfloor-\sigma}c_{1+\lfloor\sigma\rfloor}^{\sigma-\lfloor\sigma\rfloor}(1+|s|)^{\sigma}=c_{\sigma,n}(1+|s|)^{\sigma}.$$
Hence,
$$\|\mathfrak{S}_{m_s}\|_{\mathcal{L}_p(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}\leq \frac{p^2}{p-1}C_{\sigma,n}c_{\sigma,n}(1+|s|)^{\sigma},\quad \sigma>\frac{n+1}{2}.$$

Now, we improve the latter formula by using complex interpolation. Choose $\sigma\in(\frac{n+1}{2},\frac{n+1}{2}\cdot\frac{p-1}{p-2}).$ By complex interpolation,
\begin{align*}
\|\mathfrak{S}_{m_s}\|_{\mathcal{L}_p(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}&\leq \|\mathfrak{S}_{m_s}\|_{\mathcal{L}_2(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}^{\frac1{p-1}}\|\mathfrak{S}_{m_s}\|_{\mathcal{L}_{2p}(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}^{\frac{p-2}{p-1}}\\
&\leq \big(4C_{\sigma,n}c_{\sigma,n}\frac{p^2}{2p-1}\big)^{\frac{p-2}{p-1}}(1+|s|)^{\sigma\frac{p-2}{p-1}}\\
&\leq \big(4C_{\sigma,n}c_{\sigma,n}\frac{p^2}{2p-1}\big)^{\frac{p-2}{p-1}}(1+|s|)^{\frac{n+1}{2}}.
\end{align*}
This ends the proof of Lemma \ref{post-parcet lemma}.
\end{proof}}
...
Lemma \ref{post-parcet-adashed lemma} asserts that
...
\begin{proposition}\label{fkl schur lemma} If $(k,l)\in\{(2,0),(1,1),(2,1)\}$ and $1<p<\infty$, then the Schur multipliers\footnote{The functions $F_{k,l}$ are introduced in Notation \ref{fkl notation} and the functions $G_{k,l}$ are introduced at the beginning of Appendix \ref{appendix section}.} $\mathfrak{S}_{F_{k,l}\circ H}$ and $\mathfrak{S}_{G_{k,l}\circ H}$ are bounded from $\mathcal{L}_p(L_2(\mathbb{R}^{n+1}_+))$ to itself. Consequently, those Schur multipliers are bounded from $\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+))$ to itself and from $(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0$ to itself.
\end{proposition}
\begin{proof} By Theorem \ref{fkl are smooth theorem}, $F_{k,l}$ and $G_{k,l}$ satisfy the conditions in Theorem \ref{main schur theorem}. This, in combination with Theorem \ref{main schur theorem}, yields that $\mathfrak{S}_{F_{k,l}\circ H}$ and $\mathfrak{S}_{G_{k,l}\circ H}$ are bounded on $\mathcal{L}_p(L_2(\mathbb{R}^{n+1}_+)),$ $1<p<\infty.$ By interpolation, $\mathfrak{S}_{F_{k,l}\circ H}$ and $\mathfrak{S}_{G_{k,l}\circ H}$ are bounded on $\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)),$ $1<p<\infty.$

{\color{blue} Let now $V\in(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0.$ Fix $\epsilon>0$ and choose $W\in\mathcal{L}_p(L_2(\mathbb{R}^{n+1}_+))$ such that $\|V-W\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+))}<\epsilon.$ We have $\mathfrak{S}_{F_{k,l}\circ H}(W)\in \mathcal{L}_p(L_2(\mathbb{R}^{n+1}_+)).$ Hence,
\begin{align*}
{\rm dist}\Big(\mathfrak{S}_{F_{k,l}\circ H}(V),(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0\Big)&\leq \|\mathfrak{S}_{F_{k,l}\circ H}(V)-\mathfrak{S}_{F_{k,l}\circ H}(W)\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}_+^{n+1}))}\\
&\leq \|\mathfrak{S}_{F_{k,l}\circ H}\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}\|V-W\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}_+^{n+1}))}\\
&\leq\epsilon\|\mathfrak{S}_{F_{k,l}\circ H}\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}_+^{n+1}))\circlearrowleft}.
\end{align*}
Since $\epsilon>0$ is arbitrary, it follows that $\mathfrak{S}_{F_{k,l}\circ H}(V)\in (\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0.$ Since $V\in(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0$ is arbitrary, it follows that $\mathfrak{S}_{F_{k,l}\circ H}:(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0\to (\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0$ (boundedness immediately follows from that of $\mathfrak{S}_{F_{k,l}\circ H}:\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+))\to \mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+))$). That $\mathfrak{S}_{G_{k,l}\circ H}$ is bounded from $(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0$ to itself can be proved similarly.}	
\end{proof}
...
\end{align*}
Next, the Schur multiplier $\mathfrak{S}_b$ is the triangular truncation operator on the semifinite von Neumann algebra $\mathcal{M}=B(L_2(\mathbb{R}^{n+1}_+))$ with respect to the spectral measure of the operator $M_{x_{n+1}}.$ Boundedness of triangular truncation operator on
...
\end{align*}
The second assertion follows by interpolation (see a similar argument in Proposition \ref{fkl schur lemma}).
\end{proof}
...
$$\|\mathfrak{S}_{f}\|_{\mathcal{L}_{p}(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))\circlearrowleft}=\|\mathfrak{S}_{f}\|_{\mathcal{L}_{p}(L_2(\mathbb{R}^{n+1}_+,dx))\circlearrowleft},\quad\|\mathfrak{S}_{f}\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))\circlearrowleft}=\|\mathfrak{S}_{f}\|_{\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+,dx))\circlearrowleft}.$$
\end{lemma}
\begin{proof} The assertion follows from the obvious equality
$$M_{x_{n+1}^{\lambda}}\cdot\mathfrak{S}_{f}(T)\cdot M_{x_{n+1}^{-\lambda}}=\mathfrak{S}_{f}\Big(M_{x_{n+1}^{\lambda}}TM_{x_{n+1}^{-\lambda}}\Big).$$
\end{proof}
}
...
\begin{proof}[Proof of upper bound in Theorem \ref{main theorem}] {\color{blue}If $f=E^{\ast}g$ for some $g\in C_c^\infty(\mathbb{R}^{n+1})$ even in the $(n+1)$-st argument, then Lemma \ref{mta final lemma} yields
\begin{align*}
\|[R_{\lambda,k},M_f]\|_{\mathcal{L}_{n+1,\infty}(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))}&=\|[R_{\lambda,k},M_{E^{\ast}g}]\|_{\mathcal{L}_{n+1,\infty}(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))}\\
&\leq C_{n,\lambda}\|g\|_{\dot{W}^{1,n+1}(\mathbb{R}^{n+1})}\\
&=2^{\frac1{n+1}}C_{n,\lambda}\|f\|_{\dot{W}^{1,n+1}(\mathbb{R}^{n+1}_+)}.
\end{align*}
}

We now apply an approximation argument to remove the above assumption. To this end, take $f\in  \dot{W}^{1,n+1}(\mathbb{R}_+^{n+1})\cap L_{\infty}(\mathbb{R}_+^{n+1})$ and let $\{f_m\}_{m\geq 1}$ be the sequence given by Lemma \ref{density lemma}, so that $f_m\in C_c^\infty(\mathbb{R}^{n+1})$ for $m\geq 1$ and $\{f_m\}_{m\geq 1}$ is a Cauchy sequence on $\dot{W}^{1,n+1}(\mathbb{R}^{n+1})$. Using Lemma \ref{mta final lemma}, we deduce that for $m_1,m_2\geq1$,
\begin{align*}
...
&\leq C_{n,\lambda}\limsup_{m\rightarrow \infty}\|f_m\|_{\dot{W}^{1,n+1}(\mathbb{R}^{n+1})}\\
&=2^{\frac1{n+1}}C_{n,\lambda}\|f\|_{\dot{W}^{1,n+1}(\mathbb{R}_+^{n+1})}.
\end{align*}
...
{\color{blue}
\begin{lemma}\label{smoothing fact} Let $K\in C^{\infty}_c(\mathbb{R}^{n+1}_+\times\mathbb{R}^{n+1}_+)$ (this, in particular means that $K$ vanishes near the boundary). Then the integral operator $A$ with integral kernel $K$ belongs to $\mathcal{L}_1(L_2(\mathbb{R}^{n+1}_+,m_{\lambda})).$ Furthermore, its trace is given by the Fredholm formula
$${\rm Tr}_{B(L_2(\mathbb{R}^{n+1}_+,m_{\lambda}))}(A)=\int_{\mathbb{R}^{n+1}_+}K(x,x)dm_{\lambda}(x).$$
\end{lemma}
\begin{proof} The operator $B=M_{x_{n+1}^{\lambda}}AM_{x_{n+1}^{-\lambda}}$ has integral kernel
$$L:(x,y)\to (x_{n+1}y_{n+1})^{\lambda}K(x,y),\quad x,y\in\mathbb{R}^{n+1}_+.$$
Since $K\in C^{\infty}_c(\mathbb{R}^{n+1}_+\times\mathbb{R}^{n+1}_+),$ it follows that also $L\in C^{\infty}_c(\mathbb{R}^{n+1}_+\times\mathbb{R}^{n+1}_+).$ Hence, $B$ extends to a smoothing operator on $C^{\infty}(\mathbb{R}^{n+1}).$ That is, for any distribution $u\in \mathcal{D}'(\mathbb{R}^{n+1})$, $Bu\in C^\infty(\mathbb{R}^{n+1})$. Note that every smoothing operator belongs to $\mathcal{L}_1(L_2(\mathbb{R}^{n+1}))$ and its trace is computed by the Fredholm formula. Hence, $M_{x_{n+1}^{\lambda}}AM_{x_{n+1}^{-\lambda}}\in\mathcal{L}_1(L_2(\mathbb{R}^{n+1}))$ and
$${\rm Tr}_{B(L_2(\mathbb{R}^{n+1}_+))}(M_{x_{n+1}^{\lambda}}AM_{x_{n+1}^{-\lambda}})={\rm Tr}_{B(L_2(\mathbb{R}^{n+1}))}(M_{x_{n+1}^{\lambda}}AM_{x_{n+1}^{-\lambda}})=\int_{\mathbb{R}^{n+1}_+}x_{n+1}^{2\lambda}K(x,x)dx=\int_{\mathbb{R}^{n+1}_+}K(x,x)dm_{\lambda}(x).$$
Since $M_{x_{n+1}^{-\lambda}}$ is a unitary operator from $L_2(\mathbb{R}^{n+1}_+)$ to $L_2(\mathbb{R}^{n+1}_+,m_{\lambda}),$ the assertion follows.
\end{proof}}
...
\end{lemma}
\begin{proof} {\color{blue} Suppose first $f\in C_c^{\infty}(\mathbb{R}^{n+1}_+)$ (this, in particular, means $f$ vanishes near the boundary) and $g\in C_c(\mathbb{R}_+)$ (this, in particular, means $g$ vanishes near $0$). By the functional calculus \cite{Yosida},
...
$$\big(|g|(\sqrt{\Delta_{\lambda}})(h)\big)(x)=\frac{1}{(2\pi)^n}\int_{\mathbb{R}_+^{n+1}}\Big(\int_{\mathbb{R}_+^{n+1}}e^{i\langle x'-y',z'\rangle}|g|^2(|z|)\phi_{\lambda}(x_{n+1}z_{n+1})\phi_\lambda(y_{n+1}z_{n+1})dm_\lambda(z)\Big)h(y)dm_\lambda(y)$$
...
\begin{proposition}\label{final sufficiency lemma} Let $n\geq 2.$ There exists a constant $C_{n,\lambda}>0$ such that for every $f\in  \dot{W}^{1,n+1}(\mathbb{R}_+^{n+1})\cap L_{\infty}(\mathbb{R}_+^{n+1})$ and for every $1\leq k\leq n+1,$ we have
...
Let $V$ be supported on a compact set $Q\subset \mathbb{R}^{n+1}_+$ (in particular, this means $Q$ is separated from the boundary). We have
...
By Proposition \ref{fkl schur lemma}, $\mathfrak{S}_{G_{k,l}\circ H}:(\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0\to (\mathcal{L}_{p,\infty}(L_2(\mathbb{R}^{n+1}_+)))_0.$  Hence,
...
\begin{equation}\label{concrete majorization ex}
M_{\chi_Q}g(\nabla)M_{\chi_Q}\prec\prec 160\chi_Q\otimes g,\quad M_{\chi_Q}g_{k,l}(\nabla)M_{\chi_Q}\prec\prec 160\chi_Q\otimes g_{k,l}.
\end{equation}
Here we recall in Section \ref{Spdef} that $ A \prec\prec B$ means $A$ is submajorized by $B.$ {\color{blue}Note that $\mathcal{L}_{\frac{n+1}{2}}(L_2(\mathbb{R}^{n+1}))=(\mathcal{L}_1+\mathcal{L}_{\frac{n+1}{2}})(L_2(\mathbb{R}^{n+1}))$ is closed under Hardy-Littlewood submajorization. Since $\chi_Q\otimes g,\chi_Q\otimes g_{k,l}\in (L_1+L_{\frac{n+1}{2}})(\mathbb{R}^{n+1}\times\mathbb{R}^{n+1}),$ it follows from \eqref{concrete majorization ex} that
...
\begin{lemma}\label{ii pre-verification lemma} If $Q_j=[-j,j]^n\times[\frac1j,j]$ and $f\in C^{\infty}_c(\mathbb{R}^{n+1}),$ then
...
\begin{lemma}\label{ii verification lemma} If $1\leq k\leq n+1$, $Q_j=[-j,j]^n\times[\frac1j,j]$ and $f\in C^{\infty}_c(\mathbb{R}^{n+1}),$ then
...
\end{align*}
{\color{blue} Write
$$A_k(1+\Delta)^{\frac12}=\left(M_{\partial_k f}-\sum_{m=1}^{n+1}M_{\partial_m f}\frac{\partial_k\partial_m}{1+\Delta}\right)\cdot (1+\Delta)^{-\frac12}M_{\psi}(1+\Delta)^{\frac12}.$$
The first factor is obviously bounded. The second factor is a pseudo-differential operator of order $0$ and is, therefore, bounded. Hence, $A_k(1+\Delta)^{\frac12}$ is bounded.	Also, write
$$(1+\Delta)^{\frac12}A_k=(1+\Delta)^{\frac12}M_{\partial_kf}(1+\Delta)^{-\frac12}\cdot M_{\psi}-\sum_{m=1}^{n+1}(1+\Delta)^{\frac12}M_{\partial_mf}(1+\Delta)^{-\frac12}\cdot \frac{\partial_k\partial_m}{1+\Delta}M_{\psi}.$$
In each summand, the second factor is obviously bounded, while the first factor is a pseudo-differential operator of order $0$ and is, therefore, bounded. Hence, $(1+\Delta)^{\frac12}A_k$ is bounded.} The assertion follows now from Lemma \ref{ii pre-verification lemma}.
\end{proof}
...
\section{Smoothness of auxiliary functions}\label{appendix section}
\setcounter{equation}{0}
...
satisfies $d^{(j)}\in L_{\infty}(0,1)$ for $0\leq j\leq 2n+2.$
\end{lemma}
\begin{proof} {\color{blue}By the Leibniz rule,
$$d^{(j)}(x)=\sum_{\substack{j_1,j_2\geq0\\ j_1+j_2=j\\
j_1\leq n}}\binom{j}{j_1}\cdot\prod_{m=0}^{j_1-1}(n-m)x^{n-j_1}\cdot\int_0^1\big((x^2+t)^{-\lambda-\frac{n}{2}-1}\big)^{(j_2)}t^{\lambda+n+2}\psi(t)dt.$$
By homogeneity, we have
$$x^{n-j_1}\cdot\Big|\big((x^2+t)^{-\lambda-\frac{n}{2}-1}\big)^{(j_2)}\Big|\cdot t^{\lambda+\frac{j}{2}+1}\leq c(n,j_1,j_2),\quad x,t>0.$$
Hence,
$$x^{n-j_1}\cdot\Big|\big((x^2+t)^{-\lambda-\frac{n}{2}-1}\big)^{(j_2)}\Big|\cdot t^{\lambda+n+2}\leq c(n,j_1,j_2),\quad x>0,\quad t\in(0,1),$$
provided that $j\leq 2n+2.$ Thus,
$$|d^{(j)}(x)|\leq \sum_{\substack{j_1,j_2\geq0\\ j_1+j_2=j\\ j_1\leq n}}\binom{j}{j_1}\cdot\prod_{m=0}^{j_1-1}(n-m)\cdot c(n,j_1,j_2)\cdot \|\psi\|_{\infty}.$$}
...
{\color{blue}
\begin{lemma}\label{fourth fkl lemma} Let $k,l\in\mathbb{Z}_+.$ For every $j\geq0$, there is a constant $C_{n,\lambda,k,l,j}>0$ such that
$$|F_{k,l}^{(j)}(x)|\leq C_{n,\lambda,k,l,j}x^{k-2-2\lambda-j},\quad x\in (0,\infty).$$
\end{lemma}
\begin{proof}
By Leibniz's rule,
$$F_{k,l}^{(j)}(x)=\sum_{\substack{j_1+j_2\geq0\\ j_1+j_2=j\\ j_1\leq n+k}}\binom{j}{j_1}\cdot \prod_{m=0}^{j_1-1}(n+k-m)x^{n+k-j_1}\cdot \int_0^2\big((x^2+2t)^{-\lambda-\frac{n}{2}-1-j_2}\big)^{(j_2)}(2t-t^2)^{\lambda-1}t^ldt.$$
By homogeneity, we have
$$x^{2+2\lambda+j-k}\cdot x^{n+k-j_1}\cdot\Big|\big((x^2+t)^{-\lambda-\frac{n}{2}-1}\big)^{(j_2)}\Big|\leq c(n,j_1,j_2),\quad x,t>0.$$
Thus,
$$x^{2+2\lambda+j-k}\cdot|F_{k,l}^{(j)}(x)|\leq\sum_{\substack{j_1+j_2\geq0\\ j_1+j_2=j\\ j_1\leq n+k}}\binom{j}{j_1}\cdot \prod_{m=0}^{j_1-1} c(n,j_1,j_2)\cdot \int_0^2(2t-t^2)^{\lambda-1}t^ldt.$$
...
{\color{blue} Observe that $P_{k,l}(0)=P_{k,l}'(0)=0.$ Indeed, if $(k,l)\in\{(1,1),(2,1)\}$, then the power $k+2l-2+2j$ is strictly larger  than zero, which means that $P_{2,1}(0)=P_{1,1}(0)=0$. If $(k,l)=(2,0),$ then we apply Lemma \ref{third fkl lemma} to see that $a_{0,0}=0$, and therefore,
...
{\color{blue}The authors would like to thank the referees for helpful comments. The authors would like to thank Javier Parcet and Eric Ricard for proposing the ideas in Lemma 4.7. }
Z. Fan is supported by the Guangdong Basic and Applied Basic Research Foundation (No. 2023A1515110879), by the China Postdoctoral Science Foundation (No. 2023M740799) and by the Postdoctoral Fellowship Program of CPSF (No. GZB20230175). J. Li is supported by the Australian Research Council (ARC) through the research grant DP220100285. F. Sukochev and D. Zanin are supported by the Australian Research Council through the research grant DP230100434. F. Sukochev is supported by the Australian Research Council Laureate Fellowship FL170100052. Z. Fan would like to thank Lixin Yan for consistent help and thank Dongyong Yang for discussions about harmonic analysis associated with {\color{blue}the Bessel operator}.
...
\bibitem{MR4716964}
R.L. Frank.
\newblock A characterization of {$\dot W{}^{1,p}(\mathbb{R}^d)$}.
...
\bibitem{MSZ-krein}
{\color{blue}E.~McDonald, F.~Sukochev, and D.~Zanin.
\newblock Krein's theorem revisited.
\newblock {\em Pure Appl. Funct. Anal.}, 9(5):1281--1317, 2024.}
